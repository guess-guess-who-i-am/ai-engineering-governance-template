...
const executionGate = `[TOOL_BATCH_EXECUTION_GATE_V1]
...
try {
  ...
  if (eventName === "SessionStart" && input.source === "compact") parts.push("Compaction recovery: restore the active task, selected methodology routes, applicable `AGENTS.md` files, repository state, evidence, and first unresolved gap before continuing. Do not load the complete methodology archive; reload only the routes that still apply.");
  if (eventName === "UserPromptSubmit") parts.push(executionGate);
  process.stdout.write(JSON.stringify({ hookSpecificOutput: { hookEventName: eventName, additionalContext: parts.join("\n\n") } }));
} catch (error) {

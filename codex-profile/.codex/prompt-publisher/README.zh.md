...
桌面快捷方式 `编辑并发布全局 Prompt` 现在打开中文唯一源：

`~/.codex/prompts/global-methodology-source.zh.md`

这个文件按 `router`、`alwaysOn` 和五个 `method-*` 分类组织。你可以直接：

1. 在现有分类下增加、删除或修改一条 `- ` 规则。
2. 把整条规则移动到另一个分类。
3. 保存文件；等待翻译和发布完成提示。

保存后自动完成：中文到英文逐行翻译、完整档案、常驻包、路由文件、五个 Skills、归类映射、Skill 索引、字符预算和全部规则的完整性校验。翻译按短片段并行执行，英文正文残留未翻译中文会阻止发布。所有目标先备份后统一更新；后置校验失败会回滚。
...
新建第六种方法 Skill 前，先让 Agent 建立 Skill 文件、触发条件、排除条件，并把新分类登记到 `methodology-targets.json`；发布器不会凭分类名猜测触发规则。

手动检查或发布：

```powershell
node "$env:USERPROFILE\.codex\prompt-publisher\publish-methodology.mjs" --check
node "$env:USERPROFILE\.codex\prompt-publisher\publish-methodology.mjs"
```

下面是保留的旧版整包 Prompt 发布器说明。

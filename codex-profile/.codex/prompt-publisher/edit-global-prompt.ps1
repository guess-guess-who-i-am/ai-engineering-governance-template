...
$sourceBackupDirectory = if ([string]$settings.sourceBackupDirectory) {
  Resolve-ConfigPath $configDir ([string]$settings.sourceBackupDirectory)
} else { "" }
$publisher = if ([string]$settings.publisherScript) {
  Resolve-ConfigPath $configDir ([string]$settings.publisherScript)
} else {
  Join-Path $PSScriptRoot "publish-global-prompt.mjs"
}
$displayName = if ([string]$settings.displayName) { [string]$settings.displayName } else { "全局 Prompt" }
...
try {
  ...
  function Invoke-Publisher {
    Write-Status "检测到保存，开始翻译、校验和发布 $displayName。"
  ...
  if ($sourceBackupDirectory) {
    New-Item -ItemType Directory -Path $sourceBackupDirectory -Force | Out-Null
    $sourceBackup = Join-Path $sourceBackupDirectory ((Get-Date -Format "yyyyMMdd-HHmmss") + "-" + [IO.Path]::GetFileName($sourceFile))
    Copy-Item -LiteralPath $sourceFile -Destination $sourceBackup -Force
    Write-Status "打开前已备份中文源：$sourceBackup"
  }
  Write-Status "正在打开 $displayName 中文唯一源：$sourceFile"
  ...
  if ($lastError) {
    [System.Windows.Forms.MessageBox]::Show(
      "$displayName 未能完整发布。`n`n$lastError`n`n日志：$logFile",
      "$displayName 发布失败",

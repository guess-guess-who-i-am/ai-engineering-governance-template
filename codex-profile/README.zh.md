...
- 工具密集型提示的条件式执行调度契约：在 Skill 加载之后立即批量执行独立读取、搜索、状态检查和验证，优先使用一次 `functions.exec` 中的 `Promise.all` 形成 3–8 路真实并发；
...
原始的“通常直接5到8个工具调用或者进程的并发”仍逐轮注入且没有改写。除此之外，`context-refresh.ps1` 会识别工具密集型提示，并在本轮上下文末尾追加更强的执行契约；简单问答不会追加。该契约要求并发真实独立的操作，不会把存在数据依赖、交互确认、审批或破坏性的步骤伪装成并发。

...
description: Keep a Markdown task tree focused on decision-relevant current state. Use when writing or refining task-tree.md/subtrees, when a tree is too large or polluted by history or artifact details, when completed work must be distilled into parent insights, or when compact/postflight gates report size or stale-state problems. Do not use for ordinary project edits that do not change task-tree state.
---

...
不需要打开终端时，在 Finder 中右键仓库文件夹，选择“打开方式 → Deploy Codex Profile”；应用会自动识别该文件夹、弹出成功/失败提示，并把配置部署到当前用户目录。这个 Finder 入口由 macOS LaunchServices 注册，重启后仍可使用；部署一次后，该用户的所有 Codex 工作区都会使用同一份全局配置。
...
Finder 右键入口也有独立 macOS 测试，覆盖文件夹类型注册、LaunchServices 启动、进程重启后的再次部署、重复部署幂等和非仓库目录的可理解失败提示：`npm run test:codex-profile-launcher:mac`。

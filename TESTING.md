...
每个 Story 还必须完成“测试设计矩阵”，逐项决定主成功路径、业务失败与恢复、边界与重复操作、性能与容量、身份与权限、兼容与历史数据、持久数据与副作用。每项必须引用真实 AC，或写出当前可审查的 `N/A: 具体理由`。这使“正向、异常、幂等、性能、安全、兼容、底层数据”成为可校验的设计决定，而不是一句提醒。

测试与开发从需求评审后并行：测试先设计矩阵和断言，开发完成一个可独立运行的切片后交付提交、构建、环境、真实入口和开发自测。阶段性交付、跨边界验证、缺陷修复或发布候选在 `requirements/test-runs/` 留一份轻量记录；普通局部单元测试不需要额外长报告。记录必须回链真实 Story 与 AC，并观察用户结果、数据库/文件/队列/审计等适用副作用及清理方式。

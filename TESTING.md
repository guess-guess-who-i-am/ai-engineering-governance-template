...
本仓库把测试理解为一条从用户意图到可重复证据的链，而不是几个零散命令。所有新项目都继承完整测试类别，但只有从项目类型能可靠判断为相关的类别默认登记为 `planned`；其他类别保留具体启用条件，等真实技术边界出现时再启用。发布检查拒绝仍未配置的相关门禁，普通开发不会被无关类别阻塞。
...
每个 Story 只保留三个“测试设计重点”：主成功路径、最重要的失败与恢复、变更影响面。高风险和关键风险 Story 必须有非 happy AC；低中风险确实没有相关失败时可写具体 N/A。性能、安全、兼容、历史数据和底层副作用不是每个 Story 的必填清单，而是在真正相关时进入 AC；项目是否需要这些类别由 `quality/gates.json` 统一决定。

测试与开发从需求评审后并行。用例评审、正式交接和执行记录只在风险、跨边界或团队协作确实需要时启用。阶段性交付、跨边界验证、缺陷修复或发布候选在 `requirements/test-runs/` 留一份轻量记录；普通局部单元测试不需要额外长报告。记录必须回链真实 Story 与 AC，并观察适用的数据或外部副作用及清理方式。
...
当前模板能真实执行 secret scan 和 GitHub Actions 供应链检查。常见的 `web/api/cli/research` 项目默认规划依赖扫描；容器门禁只有项目实际构建或发布镜像时才启用。每个未启用类别都保留具体触发条件，不靠静默遗漏减轻负担。

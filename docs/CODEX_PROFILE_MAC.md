...
这套安装包用于把仓库中的自建方法论、每轮 Hook、全局 Skill 路由和发布器安装到 macOS 用户目录。它不会复制或修改 `auth.json`、API Key、GitHub token、cookie 或 `.env`；只在 `config.toml` 中合并实时搜索、Hooks、多 Agent、Agent 启用和每会话20并发这5个非敏感键，并保留其它设置。
...
10. **写了“20路并发”但模型仍只跑少量调用**：模糊建议不是执行约束。当前 Hook 要求先计算已知独立操作数 `N`，令 `K=min(N,20)`；当 `K=5–20` 时首批必须提交恰好 `K` 个真实调用，并通过重叠开始/结束时间判断真实并发。
